...
(define (get-index-value lst_attribute index)
  (do ([name-values (cdr lst_attribute) (cdr name-values)]
       [i 1 (+ i 1)])
    ((or (= i index) (null? (cdr name-values)))
     (cdr (car name-values)))))
...
(define (process-question listAttributes)
  (let loop ([i 0]
             [j (- (vector-length listAttributes) 1)])
    (display (+ i 1))
    (display ",您最近是否有【")
            ...
            (begin
              ...
              (let ([q_index (input_selected)])
                ...
                (let ([curr_risk (get-index-value Ai q_index)]) 
                  (display curr_risk)
                  (set! Ctx_Attributes (append Ctx_Attributes (list (list Ai_Name (get-index-name Ai q_index) curr_risk))))
...
(display "**    注意：本程序仅为演示Racket计算机语言编程使用       **")(newline)
(newline)
